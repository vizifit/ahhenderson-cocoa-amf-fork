...
- (id)init{
	if (self = [super init]){
        ...
        // Possibly reset session?? Not sure yet
		m_receivedData = nil;
		m_delegate = nil;
		m_isLoading = NO;
...
- (void)start{
    ...
    m_request.timeoutInterval = 60.0;
    m_request.cachePolicy = NSURLRequestUseProtocolCachePolicy;
    
    
    NSURLSessionDataTask *postDataTask = [m_session dataTaskWithRequest:m_request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error) {
            ...
            NSLog(@"HTTP Status Code %d", httpResp.statusCode);

            // Prelimary validation of AMF Results
                 ...
                 rangeOfString:@"application/x-amf"].location == NSNotFound)
            {
                ...
                // Additional error logic can be added here
